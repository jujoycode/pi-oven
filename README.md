...
| [pi-hwp](packages/pi-hwp) | 한글 문서(.hwpx/.hwp 5.x) 텍스트·표 추출 + 내장 이미지 첨부(비전), 의존성 제로 | `pi install npm:pi-hwp` |
| [pi-odt](packages/pi-odt) | OpenDocument(.odt) 텍스트·표 추출 + 내장 이미지 첨부(비전), 의존성 제로 | `pi install npm:pi-odt` |
